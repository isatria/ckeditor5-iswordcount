...
class ISWordCountEditing extends Plugin {
    ...
    init() {
        ...
        const charCounter = text => {
            ...
            return (normalizedText == " " && normalizedText.length == 1) ? 0 : normalizedText.length;
        };

        const updateCounterLabel = (current, max) => {
            var html = format;

            if (!config.showRemaining) {
                html = 'Characters: ' + current + '/' + max;
            }
        ...
        const updateCounter = (event, data) => {
            var charCount = 0,
                wordCount = 0,
                text;

            text = editor.getData({ trim: 'none'});

            if (text && config.showCharCount) {
                charCount = charCounter(text);
            }

            (editor.config.iswordcount || (editor.config.iswordcount = {})).charCount = charCount;
            ...
            if (config.maxCharCount > -1 && charCount > config.maxCharCount) {
                ...
                charCount -= 1;
            } else if (config.maxCharCount == -1 || charCount <= config.maxCharCount) {
                limitReached(false);
            }
            ...
            updateCounterLabel(charCount, config.maxCharCount);
        };
        ...
        editor.editing.view.document.on('keydown', (event, data) => {
            ...
            var text = editor.getData({ trim: 'none' });

            if (config.showCharCount) {
                charCount = charCounter(text) + 1;
            }

            if (config.maxCharCount > -1 && charCount > config.maxCharCount && config.hardLimit) {
                //event.stop();
                data.preventDefault();
            }
        }, { priority: 'highest' });
        ...
        editor.data.on('ready', (event, data) => {
            updateCounter(event, data);
        }, editor, null, 100);

        editor.keystrokes.set( 'Ctrl+Z', ( data, cancel ) => {
            // Prevent default (native) action and stop the underlying keydown event
            // so no other editor feature will interfere.
            cancel();
        });

        editor.keystrokes.set( 'Ctrl+Y', ( data, cancel ) => {
            // Prevent default (native) action and stop the underlying keydown event
            // so no other editor feature will interfere.
            cancel();
        });
